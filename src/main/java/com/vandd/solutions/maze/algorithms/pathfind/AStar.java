...
public class AStar extends FindingExit {
    ...
    public AStar(HeuristicStrategy heuristic) {
        super();
    ...
    @Override
    public int runPathfinder(int sleepDuration, Grid model, List<Tile> path) {
        ...
        executeAStar(sleepDuration, rootNode, targetNode, this.heuristic);
        painter.drawPath(sleepDuration, path, model);
    ...
    private void executeAStar(int sleepDuration, Node rootNode, Node targetNode, HeuristicStrategy heuristic) {
        ...
        while (!notTestedNodes.isEmpty()) {
            ...
            painter.drawTile(currentNode.getTile(), rootNode.getTile(), targetNode.getTile(), Tile.Type.VISITED, sleepDuration);
        }
    }
